 ...
 * T08 stub — curated fallback only; Behold wiring in T13.
 */
...
function mapBeholdPost(post: BeholdPost): IgPost {
  return {
    ...
    mediaUrl: post.mediaType === 'VIDEO' ? (post.thumbnailUrl ?? post.mediaUrl) : post.mediaUrl,
    px: 30,
    py: 30,
  }
}
      ...
      <div className="container">
        {loading ? (
          ...
          <div
            className="ig-rail"
            role="list"
            aria-label="Instagram posts (placeholder)"
          >
            {posts.map((post, i) => (
              <a
                key={post.id}
